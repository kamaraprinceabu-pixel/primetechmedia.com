document.addEventListener('DOMContentLoaded', () => {
  ...
  async function handleFormSubmit(
    ...
    try {
      ...
      alert(
        'Unable to save your submission right now. Please make sure the backend server is running at https://primetechmedia-com.onrender.com and try again.'
      );
    }
  }
  ...
  function renderAdminTable(submissions) {
    ...
    tbody.innerHTML = submissions
      .map((submission) => {
        ...
        const createdOn = submission.createdAt
  ...
  async function loadAdminSubmissions(type = 'all') {
    try {
      ...
      console.error('Unable to load admin submissions:', error);
      ...
      if (tbody) {
        tbody.innerHTML =
          '<tr><td colspan="6" class="text-center">Unable to load submissions.</td></tr>';
      }
    }
  }
    ...
    document
      ...
      .forEach((element) => {
        ...
        if (text.includes(query)) {
          element.style.background = query
  ...
  const clearSubmissionsBtn =
    document.getElementById('clearSubmissionsBtn');
  ...
  const adminToolbarHeader =
    document.querySelector(
      '.admin-toolbar h3'
    );

  const adminToolbarText =
    document.querySelector(
      '.admin-toolbar p'
    );
  ...
  function showSection(name) {
    ...
    if (adminToolbarHeader) {
      adminToolbarHeader.textContent =
        sectionLabels[name] || name;
    }

    if (adminToolbarText) {
      adminToolbarText.textContent =
        `Manage ${
          (
            sectionLabels[name] || name
          ).toLowerCase()
        } and review all related activity.`;
    }

    if (name === 'contact-forms') {
      loadAdminSubmissions('contact');
    }

    if (name === 'bookings') {
      loadAdminSubmissions('booking');
    }

    if (name === 'marketing') {
      loadAdminSubmissions('newsletter');
    }
  }
  ...
  clearSubmissionsBtn?.addEventListener(
    ...
    async () => {
      ...
      try {
        ...
        alert(
          'Unable to clear submissions right now.'
        );
      }
    }
  );
  ...
  document.addEventListener('click', (event) => {
    ...
    if (action === 'reply') {
      alert(
        'Reply workflow will be wired to your email provider once credentials are added.'
      );
    }

    if (action === 'note') {
      alert(
        'Notes can be expanded into a CRM or follow-up workflow in the next step.'
      );
    }
  });
